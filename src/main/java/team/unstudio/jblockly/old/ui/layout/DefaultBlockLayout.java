...
public class DefaultBlockLayout implements LayoutManager {
	...
	public void layoutContainer(Container parent) {
		if (!(parent instanceof Block))
			throw new IllegalArgumentException("Container type must be Block");
	...
	protected void prePaintTop(Block block, Path2D path, int x, int y, int width, List<Line2D> darkShadow) {
		if (block.getBlockDescriber().getHeadType() == HeadType.NEXT) {
			...
			path.lineTo(x + BlockUtils.NEXT_SLOT_OFFSET_X + BlockUtils.INSERT_WIDTH - 1,
					y + BlockUtils.NEXT_SLOT_HEIGHT - 1);
			path.lineTo(x + BlockUtils.NEXT_SLOT_OFFSET_X + BlockUtils.INSERT_WIDTH - 1, y);
			path.lineTo(x + width - 1, y);
			darkShadow.add(new Line2D.Double(x + BlockUtils.NEXT_SLOT_OFFSET_X - 1, y,
					x + BlockUtils.NEXT_SLOT_OFFSET_X - 1, y + BlockUtils.NEXT_SLOT_HEIGHT - 1));
			...
			path.lineTo(x + width - 1, y);
			darkShadow.add(new Line2D.Double(x + BlockUtils.INSERT_WIDTH - 1,
					y + BlockUtils.INSERT_OFFSET_Y + BlockUtils.INSERT_HEIGHT - 1, x,
					y + BlockUtils.INSERT_OFFSET_Y + BlockUtils.INSERT_HEIGHT - 1));
		} else {
			path.moveTo(x, y);
			path.lineTo(x + width - 1, y);
		}
	...
	protected void prePaintBranch(Block block, Path2D path, int x, int y, int width, List<Line2D> darkShadow) {
		...
		path.lineTo(x + BlockUtils.SIDE_WIDTH + BlockUtils.NEXT_OFFSET_X + BlockUtils.NEXT_WIDTH - 1,
				y + BlockUtils.NEXT_HEIGHT - 1);
		...
		path.lineTo(x + BlockUtils.SIDE_WIDTH - 1, y);
		darkShadow.add(new Line2D.Double(x + width - 1, y,
				x + BlockUtils.SIDE_WIDTH + BlockUtils.NEXT_OFFSET_X + BlockUtils.NEXT_WIDTH - 1, y));
		darkShadow
				.add(new Line2D.Double(x + BlockUtils.SIDE_WIDTH + BlockUtils.NEXT_OFFSET_X + BlockUtils.NEXT_WIDTH - 1,
						y, x + BlockUtils.SIDE_WIDTH + BlockUtils.NEXT_OFFSET_X + BlockUtils.NEXT_WIDTH - 1,
						y + BlockUtils.NEXT_HEIGHT - 1));
		darkShadow
				.add(new Line2D.Double(x + BlockUtils.SIDE_WIDTH + BlockUtils.NEXT_OFFSET_X + BlockUtils.NEXT_WIDTH - 1,
						y + BlockUtils.NEXT_HEIGHT - 1, x + BlockUtils.SIDE_WIDTH + BlockUtils.NEXT_OFFSET_X - 1,
						y + BlockUtils.NEXT_HEIGHT - 1));
		darkShadow.add(new Line2D.Double(x + BlockUtils.SIDE_WIDTH + BlockUtils.NEXT_OFFSET_X - 1, y,
				x + BlockUtils.SIDE_WIDTH - 1, y));
	}
	...
	protected void prePaintBottom(Block block, Path2D path, int x, int y, int width, List<Line2D> darkShadow,
		...
		if (next) {
			...
			darkShadow.add(
					new Line2D.Double(x + width - 1, y, x + BlockUtils.NEXT_OFFSET_X + BlockUtils.NEXT_WIDTH - 1, y));
			darkShadow.add(new Line2D.Double(x + BlockUtils.NEXT_OFFSET_X + BlockUtils.NEXT_WIDTH - 1, y,
					x + BlockUtils.NEXT_OFFSET_X + BlockUtils.NEXT_WIDTH - 1, y + BlockUtils.NEXT_HEIGHT - 1));
			darkShadow.add(new Line2D.Double(x + BlockUtils.NEXT_OFFSET_X + BlockUtils.NEXT_WIDTH - 1,
					y + BlockUtils.NEXT_HEIGHT - 1, x + BlockUtils.NEXT_OFFSET_X - 1, y + BlockUtils.NEXT_HEIGHT - 1));
	...
	protected Area prePaintInsertEndSlot(Block block, Path2D path, int x, int y, List<Line2D> darkShadow) {
		...
		Polygon p = new Polygon(
				new int[] { x + BlockUtils.INSERT_SLOT_WIDTH - 1, x, x, x + BlockUtils.INSERT_SLOT_WIDTH - 1 },
				new int[] { y, y, y + BlockUtils.INSERT_SLOT_HEIGHT - 1, y + BlockUtils.INSERT_SLOT_HEIGHT - 1 }, 4);
		return new Area(p);
	}
	...
	protected Area prePaintInsertSlot(Block block, int x, int y, int width, int height, List<Line2D> darkShadow) {
		...
		p.addPoint(x + BlockUtils.INSERT_SLOT_WIDTH - 1,
				y + BlockUtils.INSERT_SLOT_OFFSET_Y + BlockUtils.INSERT_SLOT_HEIGHT - 1);
		p.addPoint(x + BlockUtils.INSERT_SLOT_WIDTH - 1, y + height - 1);
		p.addPoint(x + width - 1, y + height - 1);
		p.addPoint(x + width - 1, y);
		darkShadow.add(new Line2D.Double(x + BlockUtils.INSERT_SLOT_WIDTH - 1, y, x + BlockUtils.INSERT_SLOT_WIDTH - 1,
				y + BlockUtils.INSERT_SLOT_OFFSET_Y - 1));
		darkShadow.add(new Line2D.Double(x + BlockUtils.INSERT_SLOT_WIDTH - 1, y + BlockUtils.INSERT_SLOT_OFFSET_Y - 1,
				x, y + BlockUtils.INSERT_SLOT_OFFSET_Y - 1));
		darkShadow.add(new Line2D.Double(x, y + BlockUtils.INSERT_SLOT_OFFSET_Y - 1, x,
				y + BlockUtils.INSERT_SLOT_OFFSET_Y + BlockUtils.INSERT_SLOT_HEIGHT - 1));
		darkShadow.add(new Line2D.Double(x + BlockUtils.INSERT_SLOT_WIDTH - 1,
				y + BlockUtils.INSERT_SLOT_OFFSET_Y + BlockUtils.INSERT_SLOT_HEIGHT - 1,
				x + BlockUtils.INSERT_SLOT_WIDTH - 1, y + height - 1));
