...
public class JBlockInsert extends JPanel {
				...
				if (dragging) {
					...
					while (c instanceof JPanel) {
						if (c instanceof JBlockInsertSlot) {
							if (((JBlockInsertSlot) c).setInsert(x -= c.getX(), y -= c.getY(), JBlockInsert.this))
								c.revalidate();
							else
						...
						if (c.getComponentAt(x -= c.getX(), y -= c.getY()).equals(c))
							break;
						else
							c = c.getComponentAt(x -= c.getX(), y -= c.getY());
					}
	...
	public boolean isCanDrag(int x, int y) {
		return new Polygon(new int[] { 0, getWidth(), getWidth(), 0 }, new int[] { 0, 0, getHeight(), getHeight() }, 4)
				.contains(x, y);
	}
