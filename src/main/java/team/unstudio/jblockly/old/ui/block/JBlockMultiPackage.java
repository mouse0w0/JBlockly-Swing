...
public class JBlockMultiPackage extends JBlockMulti {
	...
	public boolean addBlock(int x, int y, JBlock block) {
		if (new Rectangle(20, realSize.height - 5, 20, 10).contains(x, y))
			return addBranch(block);
		else if (new Rectangle(10,
				getPreferredSize().height - 10
						- (getNextBlock() == null ? 0 : getNextBlock().getPreferredSize().height - 5),
				20, 10).contains(x, y))
	...
	public void doLayout() {
		...
		if (branch != null) {
			...
			if (width < branch.getPreferredSize().width + 10)
				width = branch.getPreferredSize().width + 10;
		}
		height += 20;
		if (getNextBlock() != null) {
			...
			if (width < getNextBlock().getPreferredSize().width)
				width = getNextBlock().getPreferredSize().width;
		}
	...
	public void revalidate() {
		if (branch != null && !branch.getParent().equals(this))
			branch = null;
		super.revalidate();
	}
}
