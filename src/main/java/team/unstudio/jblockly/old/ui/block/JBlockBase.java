...
public class JBlockBase extends JBlock {
	...
	protected void paintComponent(Graphics g) {
		Color light = new Color(108, 138, 223), dark = new Color(54, 85, 168), normal = new Color(74, 107, 211);

		drawCodeBlock(g, false, false, normal, light, dark);
		/*
		 * //normal { g.setColor(normal); Polygon p = new Polygon();
		 * p.addPoint(0, 0); p.addPoint(10, 0); p.addPoint(13, 5);
		 * p.addPoint(23, 5); p.addPoint(26, 0); p.addPoint(getWidth()-1, 0);
		 * p.addPoint(getWidth()-1, getRealSize().height-1); p.addPoint(26,
		 * getRealSize().height-1); p.addPoint(23, getRealSize().height+4);
		 * p.addPoint(13, getRealSize().height+4); p.addPoint(10,
		 * getRealSize().height-1); p.addPoint(0, getRealSize().height-1);
		 * g.fillPolygon(p); }
		 * 
		 * //light { g.setColor(light); Polygon p = new Polygon(); p.addPoint(0,
		 * getRealSize().height-1); p.addPoint(0, 0); p.addPoint(10, 0);
		 * p.addPoint(13, 5); p.addPoint(23, 5); p.addPoint(26, 0);
		 * p.addPoint(getWidth()-1, 0); g.drawPolyline(p.xpoints, p.ypoints,
		 * p.npoints); }
		 * 
		 * //dark { g.setColor(dark); Polygon p = new Polygon();
		 * p.addPoint(getWidth()-1, 0); p.addPoint(getWidth()-1,
		 * getRealSize().height-1); p.addPoint(26, getRealSize().height-1);
		 * p.addPoint(23, getRealSize().height+4); p.addPoint(13,
		 * getRealSize().height+4); p.addPoint(10, getRealSize().height-1);
		 * p.addPoint(0, getRealSize().height-1); g.drawPolyline(p.xpoints,
		 * p.ypoints, p.npoints); }
		 */
	}
	...
	public boolean addBlock(int x, int y, JBlock block) {
		if (new Rectangle(10, realSize.height - 5, 20, 10).contains(x, y))
	...
	public void revalidate() {
		if (next != null && !next.getParent().equals(this))
			next = null;
		super.revalidate();
	}
}
