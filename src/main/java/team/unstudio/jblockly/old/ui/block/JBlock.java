...
public abstract class JBlock extends JPanel {
		...
		addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent arg0) {
				if (dragging) {
					...
					if (c instanceof JBlock && ((JBlock) c).addBlock(x - c.getX(), y - c.getY(), JBlock.this))
						c.revalidate();
					else
						editor.getDisplayLayer().add(JBlock.this, 0);
	...
	public boolean isCanDrag(int x, int y) {
		return new Polygon(new int[] { 0, realSize.width, realSize.width, 0 },
				new int[] { 0, 0, realSize.height, realSize.height }, 4).contains(x, y);
	}
