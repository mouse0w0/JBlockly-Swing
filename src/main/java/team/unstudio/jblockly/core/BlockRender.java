	...
	public static PathElement[] getPathElementFromSVG(String svg) {
		...
		case 'm': {
			...
			for (int i = 0; i < e.length; i++)
				e[i] = new MoveTo(points[i * 2], points[i * 2 + 1]);
			return e;
		}
		case 'L':
		case 'l': {
			PathElement e[] = new PathElement[points.length / 2];
			for (int i = 0; i < e.length; i++)
				e[i] = new LineTo(points[i * 2], points[i * 2 + 1]);
			return e;
		}
		case 'H':
		case 'h': {
		...
		case 's': {
			...
			for (int i = 0; i < e.length; i++)
				e[i] = new CubicCurveTo(points[i * 6], points[i * 6 + 1], points[i * 6 + 2], points[i * 6 + 3],
		...
		case 't': {
			...
			for (int i = 0; i < e.length; i++)
				e[i] = new QuadCurveTo(points[i * 4], points[i * 4 + 1], points[i * 4 + 2], points[i * 4 + 3]);
			return e;
		}
		case 'A':
		case 'a': {
			...
			for (int i = 0; i < e.length; i++)
				e[i] = new ArcTo(points[i * 7], points[i * 7 + 1], points[i * 7 + 2], points[i * 7 + 5],
						points[i * 7 + 6], points[i * 7 + 4] == 1, points[i * 7 + 3] == 1);
			return e;
		}
		case 'Z':
		case 'z': {
